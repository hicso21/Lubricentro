...
export const productService = {
   ...
   */
  async createProduct(
    product: { [s: string]: unknown } | ArrayLike<unknown>
  ): Promise<Product> {
  ...
  async updateProduct(
    id: any,
    updates: ArrayLike<unknown> | { [s: string]: unknown }
  ): Promise<Product> {
    ...
    try {
      ...
      if (!updateData || updateData.length === 0) {
        ...
        console.error(
          "Update succeeded but returned 0 rows - likely RLS policy issue"
        );
        throw new Error(
